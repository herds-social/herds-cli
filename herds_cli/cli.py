...
def cli(
    ...
    # `extractions share` needs the pre-resolution value: it resolves an
    # unforced "auto" to text even when piped (pipeable-URL contract).
    ...
    # Pre-resolution --format value ("auto" unless a flag/env/config forced
    # one). `extractions share` reads this to keep its stdout URL pipeable.
